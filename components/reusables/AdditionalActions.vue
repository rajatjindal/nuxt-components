<template>
  <div class="w-full text-right grid justify-items-end px-4">
    <div class="text-xs text-gray-900" v-if="!showOptions"
    ...
    <div v-on:mouseleave="timeoutShowOptions" class="z-50"
      ...
      <div v-if="showOptions"
           class="border rounded border-gray-300 bg-gray-50 flex-col text-left shadow-xxl text-gray-800 text-xs block w-fit whitespace-nowrap w-32">
        <div v-for="(action, index) in actions" class="border-b"
             :key="index">
          <nuxt-link :to="action.link"
                     v-if="action.link">
            <div class="px-4 py-1 hover:bg-gray-200 bg-gray-50">
              {{ action.title }}
            </div>
          </nuxt-link>

          <!-- to load dynamic component -->
          <div class="py-1 px-2 hover:bg-gray-200 bg-gray-50"
